...
export class SimpleSecretSharingState implements ViewState<typeof SimpleSecretSharingStateName> {
  ...
  navState: NavigationPathState;
	...
	loadShareAsDiceKeyState: LoadDiceKeyViewState | undefined;
	setLoadShareAsDiceKeyState = action( (scanDiceKeyState: LoadDiceKeyViewState | undefined) => {this.loadShareAsDiceKeyState=scanDiceKeyState} );
	...
	get pseudoRandomShares() {
		const numberOfUserDefinedShares = this.userProvidedShares.length + ( this.userSpecifiedDiceKeyToBeShared == null ? 0 : 1 );
	...
	addUserLoadedShare = action( (diceKeyAsFiniteFieldPoint: DiceKeyInFiniteFieldPointFormat) => {
		this.userProvidedShares = [
			...this.userProvidedShares.filter( s => s.x !== diceKeyAsFiniteFieldPoint.x ),
			diceKeyAsFiniteFieldPoint
		].sort( (a, b) => Number(a.x) - Number(b.x) );
	});

	removeUserLoadedShare = action( (centerLetter: FaceLetter) => {
		const x = BigInt(faceLetterToNumber0to24(centerLetter));
		this.userProvidedShares = this.userProvidedShares.filter( s => s.x !== x );
	});

	onShareAsDiceKeyLoadCompletedOrCancelled = (diceKey: DiceKey | undefined) => {
		this.setLoadShareAsDiceKeyState(undefined);
		if (diceKey == null) return;
		if (diceKey.centerFace.letter === this.userSpecifiedDiceKeyToBeShared?.centerFace.letter) {
			// FIXME -- show error message
			return;
		}
		this.addUserLoadedShare(diceKey.asShamirShareFiniteFieldPoint);
	}

	loadShareAsDiceKey = action( () => {
		this.loadShareAsDiceKeyState = new LoadDiceKeyViewState(this.navState);
	})
  ...
  constructor(
    ...
    this.navState = new NavigationPathState(parentNavState, SimpleSecretSharingStateName, () => "");
		...
		this.userSpecifiedDiceKeyToBeShared = userSpecifiedDiceKeyToSplitIntoShares;
